...
bz primarily uses Starlark configuration (`bz.star`), which provides consistency with the Bazel ecosystem. bz also supports TOML configuration via `.bz.toml` for users who prefer a simpler format. See the [TOML Config](/bz/configuration/toml/) page for details.
